import {
  ActionRowBuilder, ApplicationCommandOptionType, ButtonBuilder,
  ButtonInteraction, ButtonStyle, CommandInteraction,
...
class GiveWays {
  ...
  async recognitionBtn(interaction: ButtonInteraction) {
    ...
    // tupla com desestruturação
    const [{ title }] = interaction.message.embeds

    if (users.includes(interaction.user.id)) {
      users.push(interaction.user.id)
      return interaction.reply(`Você está participando do sorteio ${title}`)
    }

    interaction.reply({ content: `Você já está participando do sorteio ${title}`, ephemeral: true })
  }
